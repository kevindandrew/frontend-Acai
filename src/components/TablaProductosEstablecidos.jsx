...
import { eliminarProductoEstablecido } from "../axios/productos/productos";
import ModalAlerta from "../components/ModalAlerta";
import { useModalAlerta } from "../hooks/useModalAlerta";
import VerProductoEstablecido from "./VerProductoEstablecido";
import EditarProductoEstablecido from "./EditarProductoEstablecido";
import ModalConfirmacion from "./ModalConfirmacion";

export default function TablaProductosEstablecidos({ data }) {
  const [productoestablecidoVer, setProductoEstablecidoVer] = useState(null);
  const [productoestablecidoEditar, setProductoEstablecidoEditar] =
    useState(null);
  const [productoestablecidoEliminar, setProductoEstablecidoEliminar] =
    useState(null);
  const { alerta, mostrarAlerta } = useModalAlerta();

  const confirmarEliminacion = async () => {
    if (productoestablecidoEliminar) {
      try {
        const status = await eliminarProductoEstablecido(
          productoestablecidoEliminar.id_producto_establecido
        );
        if (status === 204) {
    ...
    <>
      ...
      <div className="overflow-x-auto p-4 ">
        <div className="shadow-lg ">
          <table className="min-w-full bg-white overflow-hidden rounded-lg">
            <thead className="bg-[#3bb48b] text-white">
              <tr>
                ...
                <th className="py-2 px-4">Precio_Unitario</th>
                <th className="py-2 px-4">Helado</th>
                <th className="py-2 px-4">Opciones</th>
              </tr>
            </thead>
            {productosFiltrados.map((productoestablecido, index) => {
              return (
                <tbody key={index}>
                  <tr className="border-b border-gray-200 hover:bg-gray-100 text-center">
                    <td className="py-2 px-4 ">{productoestablecido.nombre}</td>
          ...
          {productoestablecidoVer && (
            <VerProductoEstablecido
              setModalAbierto={setProductoEstablecidoVer}
              productoestablecido={productoestablecidoVer}
            />
          )}
          {productoestablecidoEditar && (
            <div className="fixed inset-0 z-50 bg-[#aea7b46d] flex items-center justify-center">
              <EditarProductoEstablecido
                setModalAbierto={setProductoEstablecidoEditar}
                productoestablecido={productoestablecidoEditar}
              />
            </div>
          )}

          {productoestablecidoEliminar && (
            <ModalConfirmacion
              mensaje={`¿Estás seguro que quieres eliminar la producto establecido "${productoestablecidoEliminar.nombre}"?`}
              onConfirmar={confirmarEliminacion}
              onCancelar={() => setProductoEstablecidoEliminar(null)}
            />
          )}
        </div>
        <ModalAlerta
