...
import VerMateriaPrima from "./VerMateriaPrima";
import EditarMateriaPrima from "./EditarMateriaPrima";
import ModalAlerta from "../components/ModalAlerta";
import { useModalAlerta } from "../hooks/useModalAlerta";
import { eliminarMateriaPrima } from "../axios/productos/materiasprimas";
import ModalConfirmacion from "./ModalConfirmacion";

export default function TablaProductosMateriasPrimas({ data }) {
  const [materiaprimaVer, setMateriaPrimaVer] = useState(null);
  const [materiaprimaEditar, setMateriaPrimaEditar] = useState(null);
  const [materiaprimaEliminar, setMateriaPrimaEliminar] = useState(null);
  const { alerta, mostrarAlerta } = useModalAlerta();

  const confirmarEliminacion = async () => {
    if (materiaprimaEliminar) {
      try {
        const status = await eliminarMateriaPrima(
          materiaprimaEliminar.id_materia_prima
        );
        if (status === 204) {
          mostrarAlerta("exito", "Materia Prima Eliminado con Éxito");
          setTimeout(() => {
            setMateriaPrimaEliminar(false);
          }, 2200);
          setTimeout(() => {
      ...
      <div className="overflow-x-auto p-4 ">
        <div className="shadow-lg ">
          <table className="min-w-full bg-white overflow-hidden rounded-lg">
            ...
            {materiasprimasFiltrados.map((materiaprima, index) => {
              return (
                <tbody key={index}>
          ...
          {materiaprimaVer && (
            <VerMateriaPrima
              setModalAbierto={setMateriaPrimaVer}
              materiaprima={materiaprimaVer}
            />
          )}
          {materiaprimaEditar && (
          ...
          {materiaprimaEliminar && (
            <ModalConfirmacion
              mensaje={`¿Estás seguro que quieres eliminar la materia prima"${materiaprimaEliminar.nombre}"?`}
              onConfirmar={confirmarEliminacion}
              onCancelar={() => setMateriaPrimaEliminar(null)}
            />
          )}
        </div>
        <ModalAlerta
